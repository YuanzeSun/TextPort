...
主要用于解决Mac（及其他苹果设备）和Windows间传输文本困难的问题（尤其是使用微信文件传输助手时，PC端和Mac端相互冲突；若使用文件传输助手网页版每次都需要手机扫码登录又十分麻烦）。
...
点击任意一条文字可以复制；点击清空历史会删除当前所有保存的文字。
...
超过 5 条的历史记录会被实际从 `messages` 表中删除，不只是前端隐藏。清空历史也会删除当前保存的全部文字。
